...
- `src/modules/settings/index.ts`: настройки человека и правило «какой сегодня день» по его часовому поясу и началу дня. Другие модули получают `SettingsReader` при сборке в `app.ts` и не читают таблицу настроек сами.
- `src/modules/goals/index.ts`: дерево целей — дело жизни, цели, этапы, шаги. Владение проверяется в том же запросе, который ищет строку, поэтому чужая строка не находится, а не находится и отклоняется.
...
`website` (Astro SSG, SSR по необходимости) владеет публичными SEO-страницами и превью: лендингом, контентом и каталогом. `webapp` (React CSR) владеет кабинетами, checkout, панелями и настройками после входа. Маркетплейсу обычно нужны оба с `@dilife/contracts`.
...
- Zod-схемы берутся из `@dilife/contracts`.
...
Auth сайта ограничен малыми публичными функциями, например состоянием входа в шапке. Не копируй кабинет из `webapp`. При подключении API/DTO добавь `@dilife/contracts` и проверь обе стороны.
...
PostgreSQL запускает Docker Compose. Сервис разработки использует `postgres:18-alpine`, БД `dilife`, порт `54329` и том `postgres_18_data`. Тестовый сервис использует тот же образ и `dilife_test`; исполнители задают вычисленный по репозиторию `POSTGRES_TEST_PORT`.
