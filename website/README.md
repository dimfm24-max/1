...
> **Статус в DiLife: отложен (2026-09-21).** Публичные страницы и поисковая выдача в первой версии не нужны: продукт живёт после входа в `webapp` и мобильном приложении. Приложение не настраивается и не покрывается проверками. Когда публичный сайт понадобится, обнови `CHECKLIST.md`, настрой `website` и замени это примечание.
...
При подключении API/DTO явно добавь `@dilife/contracts` и проверь обе стороны контракта. `@astrojs/react` нужен только для интерактивных React-islands.
