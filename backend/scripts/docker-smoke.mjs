...
const imageName = process.env.BACKEND_DOCKER_SMOKE_IMAGE ?? 'vibe-backend:smoke'
const containerName =
  process.env.BACKEND_DOCKER_SMOKE_CONTAINER ??
  `vibe-backend-smoke-${repositoryHash}-${process.pid}`
...
const databaseUrlForContainer =
  ...
  'postgresql://superuser:superpassword@postgres_test:5432/vibe_test?schema=public'
