...
- Рабочая область: `/app`, `/app/day`, `/app/goals`, `/app/profile`, `/app/settings`.
...
- `src/features/day`: день. Запись по задаче возвращает эту задачу, и она переносится в тот день, который назвал сервер: перенос убирает её из прежнего дня тем же обновлением кэша.
